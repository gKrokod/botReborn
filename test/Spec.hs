...
import qualified Handlers.Dispatcher (Handle(..), getMessage, dispatcher, watcherForNewMessage)   
import qualified Handlers.Logger (Handle(..), logMessage) 
import qualified Data.Text as T
import qualified Data.Map.Strict as Map (lookup, insert, Map, fromList)
...
main = hspec $ do
  ...
  describe "Dispatcher logic" $ do
	...
	evalState (Handlers.Dispatcher.dispatcher dispatcherHandle >>
	           Handlers.Base.findUser baseHandle newUser) testBase
	  `shouldBe` Just (cRepeatCount testConfig)
	
	evalState (Handlers.Dispatcher.dispatcher dispatcherHandle >>
	           Handlers.Base.findUser baseHandle testForkImitation) testBase
	...
	evalState (Handlers.Dispatcher.dispatcher dispatcherHandle >>
	           Handlers.Base.findUser baseHandle newUser) testBase2
	  `shouldBe` Just repeatCount 
	
	evalState (Handlers.Dispatcher.dispatcher dispatcherHandle >>
	           Handlers.Base.findUser baseHandle testForkImitation) testBase2
	  `shouldBe` Nothing 
